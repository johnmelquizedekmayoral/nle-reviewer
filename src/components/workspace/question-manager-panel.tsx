...
export function QuestionManagerPanel({ categories, questions, onRefresh }: Props) {
  ...
  async function runBulk(action: "show" | "hide" | "move" | "delete", categoryId?: string) {
    ...
    if (error) setMessage(error.message); else {
      setMessage(`${data ?? selectedIds.size} questions updated.`); setSelectedIds(new Set()); await onRefresh();
    }
    setBusy(false);
  }
      ...
      <section className="question-manager-layout">
        ...
        <div className="question-manager-content">
          ...
          <section className="bulk-toolbar">
            ...
            <select aria-label="Move selected questions" defaultValue="" disabled={busy || !selectedIds.size} onChange={(event) => { if (event.target.value) runBulk("move", event.target.value); event.target.value = ""; }}><option value="">Move to…</option>{categoryOptions.map((category) => <option key={category.id} value={category.id}>{category.path}</option>)}</select>
          ...
          <div className="question-list local-question-list">
            {visibleQuestions.map((question) => <article className="question-row" key={question.id}>
              ...
              <div className="question-copy"><strong>{question.question_text}</strong><p>{categories.find((category) => category.id === question.category_id)?.path ?? "Uncategorized"}</p></div>
